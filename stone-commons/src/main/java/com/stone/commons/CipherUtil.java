...
public class CipherUtil {
     ...
     * HmacMD5算法默认key
     */
    private static final String ALGORITHM_HMACMD5_DEFAULT_KEY = "chenjun";
    /** 
     * HmacSHA1算法
     */ 
     ...
     * @return 返回加密后的摘要信息
     */
    private static byte[] encryptEncode(String algorithm, String data) {
        try {
     ...
     * @return 返回加密后的信息
     */
    public static byte[] SHAEncrypt(String data) {
    ...
    public static String generateMACKey() throws Exception {
        ...
        return BASE64Encrypt(secretKey.getEncoded());
    }
    
    public static String generateMACKey(String algorithm)throws Exception {
     ...
     * @throws Exception 
     */
    public static byte[] HMACEncrypt(byte[] data, String key) throws Exception {
        Key k = toKey(BASE64Decrypt(key),ALGORITHM_HMACMD5);
     ...
     * @throws Exception 
     */
    public static byte[] HMACEncrypt(byte[] data) throws Exception {
        Key k = toKey(BASE64Decrypt(ALGORITHM_HMACMD5_DEFAULT_KEY),ALGORITHM_HMACMD5);
        Mac mac = Mac.getInstance(k.getAlgorithm());
        mac.init(k);
        return mac.doFinal(data);
    }
    ...
    public static String generateDESKey(String seed) throws Exception {
        ...
        SecureRandom secureRandom = new SecureRandom(BASE64Decrypt(seed));  
     ...
     * @throws Exception 
     */
    private static byte[] DESCipher(byte[] data, String key, int mode) throws Exception {
        Key k = toKey(BASE64Decrypt(key),ALGORITHM_DES);
     ...
     */
    public static byte[] generatePBESalt() {
     ...
     * @throws Exception 
     */
    private static byte[] PBECipher(byte[] data,String password,byte[] salt,int mode) throws Exception {
     ...
     * @throws Exception 
     */
    private static Key toPBEKey(String password) throws Exception {
    ...
    /**
     * 生成AESkey
     * @param keySize key的位数,128,192,256
     * @param seed 随机种子
    ...
    private static String AESCipher(String data, String key, String algorithmParameter,int mode) {
        try {
            Key k = toKey(key.getBytes(),KEY_AES);
            ...
            return mode==Cipher.DECRYPT_MODE?new String(ecipher.doFinal(data.getBytes())):new String(ecipher.doFinal(data.getBytes()));
        } catch (Exception e) {
            e.printStackTrace();
        } 
        return null;
    ...
    private static SecretKey getDHSecretKey(String publicKey, String privateKey) {
        try {
            ...
            PublicKey pubKey = keyFactory.generatePublic(x509KeySpec);
            // 初始化私钥
            byte[] priKeyBytes = privateKey.getBytes();
